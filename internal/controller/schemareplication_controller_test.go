...
var _ = Describe("schema-replication-controller", func() {
	...
	BeforeEach(func() {
		...
		Expect((&controller.TopologyReconciler{
			...
			ReconcileFunc:         &controller.SchemaReplicationReconciler{Client: schemaReplication.GetClient()},
	...
	When("a schema replication uses vault as secretBackend", func() {
		...
		It("set schema sync parameters with generated correct endpoints", func() {
			...
			Expect(parameter).To(Equal(controller.SchemaReplicationParameterName))
