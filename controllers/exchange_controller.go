...
func (r *ExchangeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	rmq, err := internal.ParseRabbitmqClusterReference(ctx, r.Client, exchange.Spec.RabbitmqClusterReference, exchange.Namespace)
	...
	credentialsProvider, svc, err := r.ClusterCredentialsProvider.GetCredentialsProvider(ctx, exchange.Namespace, rmq)
	if err != nil {
		return reconcile.Result{}, err
	}
