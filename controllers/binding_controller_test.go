	...
	When("creating a binding", func() {
		...
		When("the RabbitMQ Client fails to generate", func() {
			When("the cluster cannot be found", func() {
				BeforeEach(func() {
					bindingName = "test-binding-client-not-found-error"
				})
				JustBeforeEach(prepareNoSuchClusterError)
			...
			When("there is a generic error", func() {
				BeforeEach(func() {
					bindingName = "test-binding-client-generic-error"
				})
				JustBeforeEach(prepareClientError)
				...
				It("sets the status condition to indicate a failure to reconcile", func() {
					...
					EventuallyWithOffset(1, func() []topology.Condition {
						...
						"Reason":  Equal("FailedCreateOrUpdate"),
						"Status":  Equal(corev1.ConditionFalse),
						"Message": ContainSubstring("such a golang error"),
					})))
				})
			})
		})
		...
		When("Deleting a binding", func() {
			...
			When("the RabbitMQ Client fails to generate", func() {
				...
				When("there is a generic error", func() {
					BeforeEach(func() {
						bindingName = "delete-binding-client-generic-error"
					})
					JustBeforeEach(prepareClientError)
