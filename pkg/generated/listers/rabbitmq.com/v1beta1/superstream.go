...
type SuperStreamLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.SuperStream, err error)
...
// List lists all SuperStreams in the indexer.
func (s *superStreamLister) List(selector labels.Selector) (ret []*v1beta1.SuperStream, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.SuperStream))
	})
	return ret, err
}
...
type SuperStreamNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.SuperStream, error)
...
// List lists all SuperStreams in the indexer for a given namespace.
func (s superStreamNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.SuperStream, err error) {
...
// Get retrieves the SuperStream from the indexer for a given namespace and name.
func (s superStreamNamespaceLister) Get(name string) (*v1beta1.SuperStream, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("superstream"), name)
	}
	return obj.(*v1beta1.SuperStream), nil
}
